...
public class ProductsFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        btn_save = root.findViewById(R.id.saveProducts);
        ...
        txt_totalToRecive = root.findViewById(R.id.totalToRecive);
    ...
    private void getAllProducts(String b){
        ...
        if(!b.isEmpty()){
            ...
            final double[] priceForProduct = new double[1];

            String url = "https://192.168.43.127/Sentencias/getProductosearch.php?idUser=" + idUser +"&descripcion="+b;
            ...
            JsonArrayRequest jsonArrayRequest = new JsonArrayRequest(url, (response) -> {
                ...
                for (int i = 0; i < response.length(); i++) {
                    try {
                        ...
                        priceForProduct[0] = jsonObject.getDouble("precio_total");
                        tot[0] = totaToProducts(priceForProduct[0]);
                        txt_totalToRecive.setText("Total a Recibir($): "+Double.toString(tot[0]));
                        try {
                            addProductsRecyclerView(idProduct[0], idMaterial[0], description[0], weight[0], priceForProduct[0]);
                        } catch (Exception e) {
            ...
            String url = "https://192.168.43.127/Sentencias/getProductsRecycler.php?id=" + idUser;
            JsonArrayRequest jsonArrayRequest = new JsonArrayRequest(url, (response) -> {
                JSONObject jsonObject = null;
                for (int i = 0; i < response.length(); i++) {
                    try {
                        ...
                        priceForProduct[0] = jsonObject.getDouble("precio_total");
                        tot[0] = totaToProducts(priceForProduct[0]);
                        txt_totalToRecive.setText(Double.toString(tot[0]));
                        try {
                            addProductsRecyclerView(idProduct[0], idMaterial[0], description[0], weight[0], priceForProduct[0]);
                        } catch (Exception e) {
                            // No encontro la ubicacion
    ...
    public void addProductsRecyclerView(int idProduct, int idMaterial , String descripcion , double peso, double priceProduct){
        ...
        products.add(new ListProducts(idProduct, imageProduct, descripcion, peso , priceProduct));
    ...
    private void generateRequest(){
        ...
        int idUser = sharedPreferences.getInt("id",0);
        double total_recibir = Double.parseDouble(txt_totalToRecive.getText().toString());

        builderDialog.setTitle("¿Estás seguro de confirmar la petición?");
        builderDialog.setMessage("Al confirmar se realizará la petición de reciclaje");
        ...
        builderDialog.setPositiveButton("Confirmar", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                String url="https://192.168.43.127/Sentencias/generateRequest.php?idUser="+idUser+"&total="+total_recibir;
    ...
    private double totaToProducts(double total){
